...
const PLANS = [
  { name: 'Starter', audience: 'Freelancers & brands', monthly: 49, yearly: 39, features: [['Projects', '3 included'], ['Seats', '3 included'], ['Credits', '500'], ['Extra project', '+$9/mo'], ['Extra seat', '+$9/mo'], ['MCP/API', true], ['Client invite', false]] },
  { name: 'Studio', audience: 'Mid-size teams', monthly: 129, yearly: 103, features: [['Projects', '10 included'], ['Seats', '10 included'], ['Credits', '2000'], ['Extra project', '+$9/mo'], ['Extra seat', '+$9/mo'], ['MCP/API', true], ['Client invite', true]] },
  { name: 'Agency', audience: 'Large teams', monthly: 349, yearly: 279, features: [['Projects', 'Unlimited'], ['Seats', 'Unlimited'], ['Credits', '6000'], ['MCP/API', true], ['Client invite', true], ['Priority support', true], ['Demo call', true]] },
] as const
...
export default function Pricing() {
  ...
  return <section id="pricing" className="pricing" ref={root} aria-labelledby="pricing-title">
    <h2 id="pricing-title">A plan for your next chapter.</h2>
    ...
    <div className="pricing-cards">{PLANS.map(plan => <article key={plan.name} className={`pricing-card pricing-card--${plan.name.toLowerCase()}`}>
      ...
      <ul className="pricing-features">{plan.features.map(([label, value]) => <li key={label}><span>{label}</span>{typeof value === 'boolean' ? <img src={`/pricing/${value ? 'included' : 'excluded'}.svg`} width="12" height="12" alt={value ? 'Included' : 'Not included'} /> : <strong>{value}</strong>}</li>)}</ul>
